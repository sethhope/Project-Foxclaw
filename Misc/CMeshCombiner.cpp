...
// Combine meshes (The one using IMeshSceneNode* may be more useful since it applies transformations)
irr::scene::IMesh* CMeshCombiner::combineMeshes(irr::video::IVideoDriver *driver, irr::core::array<irr::scene::IMesh*> meshes, bool useHardwareMappingHint) {
	...
	for (int x = 0; x < meshes.size(); x++) {
		for (int y = 0; y < meshes[x]->getMeshBufferCount(); y++) {
			indexTest += meshes[x]->getMeshBuffer(y)->getVertexCount();
			bool newMb = false;
			...
			if (!newMb) {
				irr::video::ITexture* tex = meshes[x]->getMeshBuffer(y)->getMaterial().getTexture(0);
				if (tex != 0) {
					bool copy = false;
	...
	irr::video::SMaterial newMaterial = meshes[0]->getMeshBuffer(0)->getMaterial();
	...
	for (int x = 0; x < meshes.size(); x++) {
		for (int y = 0; y < meshes[x]->getMeshBufferCount(); y++) {
			...
			int texID = -1;
			for (int z = 0; z < textures.size(); z++) {
				if (meshes[x]->getMeshBuffer(y)->getMaterial().getTexture(0) == textures[z]) {
					texID = z;
					break;
				}
			}

			irr::scene::IMeshBuffer* currentMeshBuffer = meshes[x]->getMeshBuffer(y);
...
irr::scene::IMesh* CMeshCombiner::combineMeshes(irr::video::IVideoDriver *driver, irr::core::array<irr::scene::IMeshSceneNode*> nodes, bool clearNodesAfterUse, bool useHardwareMappingHint) {
